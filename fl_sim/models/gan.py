...
class ResNetDiscriminator(nn.Module):
    def __init__(self, num_features=128, image_size=32, channels=3, cond_dim=0, num_classes=0, use_sn=True, embed_class=True):
        ...
        self.model = nn.Sequential(*self.model, nn.ReLU(), nn.AvgPool2d(8))
...
class PathwaysResNetGAN(nn.Module):
    def __init__(self, num_latents,
                 D_features=128, G_features=128, content_to_style_features=1, style_map_depth=8,
                 image_size=64, channels=3, cond_dim=0, num_classes=0, embed_class=True):
        super().__init__()
        self.num_latents = num_latents
...
class LinearPathwaysResNetGAN(nn.Module):
